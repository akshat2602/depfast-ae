...
namespace janus
{
    ...
    REG_FRAME(MODE_SAUCR, vector<string>({"saucr"}), SaucrFrame);
    ...
    SaucrFrame::~SaucrFrame() {}

    Coordinator *SaucrFrame::CreateCoordinator(cooid_t coo_id,
                                               Config *config,
                                               int benchmark,
                                               ...
                                               shared_ptr<TxnRegistry> txn_reg)
    {
        ...
        auto coord = new SaucrCoordinator(coo_id, benchmark, ccsi, id);
        setupCoordinator(coord, config);
        Log_debug("create new coord, coo_id: %d", (int)coord->coo_id_);
        return coord;
    }

    TxLogServer *SaucrFrame::CreateScheduler()
    {
        if (svr_ == nullptr)
        {
        ...
        Log_debug("create saucr sched loc: %d", this->site_info_->locale_id);

#ifdef SAUCR_TEST_CORO
        saucr_test_mutex_.lock();
    ...
    Communicator *SaucrFrame::CreateCommo(PollMgr *poll)
    {
        if (commo_ == nullptr)
        {
            commo_ = new SaucrCommo(poll);
        }
        return commo_;
    }

    vector<rrr::Service *>
    SaucrFrame::CreateRpcServices(uint32_t site_id,
    ...
    void SaucrFrame::setupCoordinator(SaucrCoordinator *coord, Config *config)
    {
        coord->frame_ = this;
