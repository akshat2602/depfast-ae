...
//     SaucrTestConfig::SaucrTestConfig(EpaxosFrame **replicas_)
//     {
...
//     void SaucrTestConfig::Start(int svr, int cmd, string dkey)
//     {
...
//     void SaucrTestConfig::GetInstance(int svr, int cmd, uint64_t *replica_id, uint64_t *instance_no)
//     {
...
//     void SaucrTestConfig::Prepare(int svr, uint64_t replica_id, uint64_t instance_no)
//     {
//         replicas[svr]->svr_->Prepare(replica_id, instance_no);
//     }

//     void SaucrTestConfig::PauseExecution(bool pause)
//     {
//         for (int i = 0; i < NSERVERS; i++)
//         {
...
//     vector<int> SaucrTestConfig::GetExecutedCommands(int svr)
//     {
//         return committed_cmds[svr];
//     }

//     int SaucrTestConfig::NExecuted(uint64_t tx_id, int n)
//     {
...
//     // Akshat: Check if this is redundant
//     bool SaucrTestConfig::ExecutedPairsInOrder(vector<pair<uint64_t, uint64_t>> expected_pairs)
//     {
...
//     // Akshat: Check if this logic can be reused for Saucr
//     bool SaucrTestConfig::ExecutedInSameOrder(unordered_set<uint64_t> dependent_cmds)
//     {
...
//     // Akshat: Change logic for this function according to the protocol
//     int SaucrTestConfig::NCommitted(uint64_t replica_id, uint64_t instance_no, int n)
//     {
//         bool cnoop;
//         string cdkey;
//         uint64_t cseq;
...
//     // Akshat: THIS FUNCTION IS REDUNDANT, REMOVE THIS ONCE THE ABOVE FUNCTION IS CHANGED
//     int SaucrTestConfig::NCommitted(uint64_t replica_id,
//                                      uint64_t instance_no,
//                                      int n,
//                                      bool *cnoop,
//                                      string *cdkey,
...
//                 if (state_ == SaucrCommandState::COMMITTED || state_ == EpaxosCommandState::EXECUTED)
//                 {
//                     nc++;
//                     if (init)
//                     {
...
//     int SaucrTestConfig::NAccepted(uint64_t replica_id, uint64_t instance_no, int n)
//     {
//         auto start = chrono::steady_clock::now();
//         int na = 0;
//         while ((chrono::steady_clock::now() - start) < chrono::seconds{2})
...
//                 map<uint64_t, uint64_t> deps_;
//                 status_t state_;
//                 // Akshat: Change getstate here
//                 GetState(j, replica_id, instance_no, &cmd_, &dkey_, &seq_, &deps_, &state_);
//                 // Akshat: Change the state enum here
//                 if (state_ == SaucrCommandState::ACCEPTED)
//                 {
//                     na++;
//                 }
//             }
...
//     int SaucrTestConfig::DoAgreement(int cmd,
//                                       string dkey,
//                                       int n,
//                                       bool retry,
//                                       bool *cnoop,
...
//             // Akshat: Change the NCommitted function definiton according to protocol
...
//     bool SaucrTestConfig::AnySlow(void)
//     {
...
//     void SaucrTestConfig::SetSlow(int svr, int msec)
//     {
//         verify(svr >= 0 && svr < NSERVERS);
//         std::lock_guard<std::mutex> prlk(disconnect_mtx_);
//         verify(!slow_[svr]);
...
//     void SaucrTestConfig::ResetSlow(int svr)
//     {
//         verify(svr >= 0 && svr < NSERVERS);
//         std::lock_guard<std::mutex> prlk(disconnect_mtx_);
//         verify(slow_[svr]);
