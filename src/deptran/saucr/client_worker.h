...
#define Print(format, ...) fprintf(stderr, format "\n", ##__VA_ARGS__)
    ...
    class SaucrClientWorker : public ClientWorker
    {
    ...
    public:
        ...
        {
#ifdef SAUCR_TEST_CORO
            testconfig_ = new SaucrTestConfig(SaucrFrame::replicas_); // todo: destroy
            Coroutine::CreateRun([this]()
                                 {
      SaucrTest test(testconfig_);
      test.Run();
      Coroutine::Sleep(10);
